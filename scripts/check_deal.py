...
if r.status_code == 200:
    ...
    for b in bids:
        amount = b.get('proposedAmount', '?')
        status = b.get('status', '?')
        title = b.get('job', {}).get('title', '?')
        print('Bid: $' + str(amount) + ' | ' + status + ' | ' + title)

# Check profile
r = requests.get(f'{DW}/profile', headers=H, timeout=15)
if r.status_code == 200:
    p = r.json()
...
# Check my listings
r = requests.get(f'{DW}/listings/me', headers=H, timeout=15)
if r.status_code == 200:
...
# Look for new jobs
r = requests.get(f'{DW}/jobs?per_page=10&sort=latest', headers=H, timeout=15)
if r.status_code == 200:
